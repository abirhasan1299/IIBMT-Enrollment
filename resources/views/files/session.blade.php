...
@section('content')
    ...
    <div class="d-flex justify-content-between">
        <a href="{{route('sessions.add')}}" class="btn btn-primary" role="button">+ Add Session</a>
        <div>
        ...
        <table class="table table-hover align-middle mb-0" id="myTable">
            ...
            @foreach($session as $s)
                <tr>
                    <td>{{$count++}}</td>
                    <td>{{$s->session}}</td>
                    <td>{{$s->courses->name}}{{$s->courses->code}}</td>
                    <td>{{$s->students_count}}</td>
                    <td class="text-center">
                        <a class="btn btn-sm btn-primary me-2" href="{{route('sessions.list',$s->id)}}">List</a>
